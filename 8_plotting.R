...
cowplot::plot_grid(p_y, p_u, p_r, p_h, nrow = 1, labels = 'AUTO', align = 'hv')
...
time_data_dec_model %>%
  ...
  geom_boxplot(aes(fill = dataset, group = interaction(workers, dataset)), position = position_dodge(width = 0),outlier.shape = '.', size = .1) +
...
ggsave_wrapper('running_time_data_dec', width = full_page, height = full_page*2/3)

lgmr_time %>%
  ggplot(aes(p, time, group = interaction(p, dataset))) +
...
#### Tables ####
paste0(c('yeast', 'ups', 'ramus', 'human'), '_pars') %>%
  set_names(., c('Yeast', 'UPS', 'Ramus', 'Human')) %>%
  map(get) %>%
  map(
    ~ c(
...
paste0(c('yeast', 'ups', 'ramus', 'human'), '_sin_gam') %>%
  set_names(., .) %>%
  map(get) %>%
